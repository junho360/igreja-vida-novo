...
import { listDiasDiaria, formatDiaDiaria } from '@/lib/dias-diaria'
...
export default function InscricaoForm({
  ...
  const [opcao, setOpcao] = useState<
    'individual' | 'dupla' | 'especie' | 'cartao' | 'diaria' | null
  >(null)
  ...
  const [diaDiaria, setDiaDiaria] = useState<string | null>(null)
  ...
  const diasDiaria = useMemo(
    () =>
      temDiaria && diariaValor != null
        ? listDiasDiaria(dataInicio ?? null, dataFim ?? null)
        : [],
    [temDiaria, diariaValor, dataInicio, dataFim]
  )
  const temDiariaAtiva =
    temDiaria && diariaValor != null && diariaValor > 0 && diasDiaria.length > 0
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (opcao === 'diaria' && !diaDiaria) {
      setErroSelecao('Escolha o dia em que você vai participar.')
      return
    }
    setErroSelecao('')
    setLoading(true)
        ...
        loteId:
          opcao === 'diaria'
            ...
            : temDoisValores
              ? null
              : (loteAtual?.id ?? null),
        diaDiaria: opcao === 'diaria' ? diaDiaria : null,
      }),
    })
          ...
          {temDiariaAtiva && (
            <div className="rounded-lg border border-primary/20 bg-primary/5 p-4 space-y-2">
              <label className="flex items-center gap-2 rounded-lg border border-gray-300 p-3 cursor-pointer has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                ...
                />
                <span className="text-sm text-gray-700">
                  Participar por dia (Diária) - R$ {diariaValor!.toFixed(2)}/dia
                  (PIX)
                </span>
              </label>
              {opcao === 'diaria' && (
